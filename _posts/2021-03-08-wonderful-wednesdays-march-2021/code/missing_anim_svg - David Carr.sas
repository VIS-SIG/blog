...
/*
options papersize=('24cm', '18cm') printerpath=gif animation=start 
animduration=0.01 animloop=yes noanimoverlay;
...
%do inc = 0 %to &maxday.;
    ...
    proc sgpanel data=&dset. noautolegend;
	     ...
	     inset txttime / position=topleft nolabel textattrs=(size=40 weight=bold color=lightgray);

         colaxis display=(noticks novalues nolabel) offsetmin=0.05 offsetmax=0.08 min=1 max=&maxord.;

         rowaxis values=(0 to 70 by 10) offsetmin=0.05 offsetmax=0.05 grid values=(10 to 70 by 10) 
               label='Pain'  valueattrs=(size=8) labelattrs=(size=9); 
...
data alrs_st0(keep=treat subjid aval_first);
     set alrs1;
     by treat subjid;
     if first.subjid;
     aval_first=aval;
...
%graph(dset=tst3,x=ordnr,fname=missing_anim_svg);
